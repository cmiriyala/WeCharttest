...
class NavigationController extends Controller
{
            ...
            //     ->where('navigation_id', '31') ->where('doc_control_id', '81')->get();
                ...
                $results = active_record::where('patient_id', $id)
                    ...
                    $status = users_patient::where('patient_id',$id)->where('user_id',$user_id)->first();
                            ...
                            if($status) {
                                $status_id = $status->patient_record_status_id;
                                return view('patient/HPI', compact ('status_id','HPI','patient','navs','vital_signs_header','disposition','videos','pictures','audios','labs','images','procedures','medications','dosage','i','results','diagnosis_list_ddx'));
                            }
                            else
                            {
                                $error_message= "Student can only view their created patients. You are not authorized to view this page.";
    ...
    public function get_medications($id)
    {
            ...
            //     ->where('navigation_id', '31') ->where('doc_control_id', '81')->get();
                ...
                $medications_main = active_record::leftjoin('active_record as act2', 'active_record.active_record_id', '=', 'act2.doc_control_group')
                    ->where('active_record.patient_id', '=', $id)
                    ->where('active_record.navigation_id', '=', '7')
                    ->where('active_record.doc_control_id', '=', '16')
                    ->whereNull('active_record.doc_control_group')
                    ->select('active_record.*','act2.value as dosage')->get();
                //Log::info($medications);
            ...
            $status = users_patient::where('patient_id',$id)->where('user_id',$user_id)->first();
             if($status) {
                $status_id = $status->patient_record_status_id;
                return view('patient/medications', compact ('status_id','vital_signs_header','medications','medication_comment','patient','navs','disposition','videos','pictures','audios','labs','images','procedures','medications_main','dosage','i','results','diagnosis_list_ddx'));
    ...
    public function get_vital_signs($id)
    {
            ...
            $status = users_patient::where('patient_id',$id)->where('user_id',$user_id)->first();
               ...
               if($status) {
                    $status_id = $status->patient_record_status_id;
            return view('patient/vital_signs', compact('status_id','vital_signs_header','patient','navs','vital_sign_details','disposition','videos','pictures','audios','labs','images','procedures','medications','dosage','i','results','diagnosis_list_ddx'));
            ...
            $status = users_patient::where('patient_id',$id)->where('user_id',$user_id)->first();
                if($status) {
                    ...
                    return view('patient/physical_exams', compact ('status_id','navIds',
                        'vital_signs_header','patient','navs','disposition','videos','pictures','audios','labs','images','procedures','medications','dosage','i','results','diagnosis_list_ddx','neurological_symptoms',
    ...
    public function get_ROS($id)
    {
           ...
           $ros_gastrointestinal_comment = active_record::where('patient_id', $id)
            ...
            $status = users_patient::where('patient_id',$id)->where('user_id',$user_id)->first();
                 if($status) {
                    $status_id = $status->patient_record_status_id;
                    return view('patient/review_of_system', compact ('navIds','vital_signs_header','patient','navs','disposition','videos','pictures','audios','labs','images','procedures','medications','dosage','i','results','diagnosis_list_ddx',
...
public function get_ROS_gastrointestinal_symptoms($id)
{
    ...
    public function get_orders($id)
    {
            ...
            $status = users_patient::where('patient_id',$id)->where('user_id',$user_id)->first();
                if($status) {
                    $status_id = $status->patient_record_status_id;
                    return view('patient/orders', compact ('status_id','vital_signs_header','patient','navs','labs','images','procedures','comment_order','disposition','videos','pictures','audios','medications','dosage','i','results','diagnosis_list_ddx'));
                }
                else
                {
                    $error_message= "Student can only view their created patients. You are not authorized to view this page.";
    ...
    public function get_results($id)
    {
            ...
            if($status) {
                $status_id = $status->patient_record_status_id;
                return view('patient/results', compact ('status_id','vital_signs_header','labs','procedures','images','results','patient','navs','disposition','videos','pictures','audios','medications','dosage','i','diagnosis_list_ddx'));
            }
            else
            {
                $error_message= "Student can only view their created patients. You are not authorized to view this page.";
    ...
    public function get_MDM($id)
    {
            ...
            $status = users_patient::where('patient_id',$id)->where('user_id',$user_id)->first();
                if($status) {
                    ...
                    return view('patient/MDM', compact ('MDM','patient','navs','vital_signs_header','disposition', 'status_id','videos','pictures','audios','labs','images','procedures','medications','dosage','i','results','diagnosis_list_ddx'));
                }
                else
                {
                    $error_message= "Student can only view their created patients. You are not authorized to view this page.";
    ...
    public function get_studentpreview($id){
            ...
            elseif (!$patient->completed_flag) {
                ...
                $feedbacks = users_patient::join('users','users.id','=','users_patient.user_id')->where('patient_id',$id)->
                whereNotNull('feedback')->select('users_patient.feedback as feedback','users.firstname as firstname','users.lastname as lastname')->get();
                ...
                return view('patient/studentpreview', compact('student_details','instructor_Details', 'patient', 'navs', 'feedbacks',
                    ...
                    'social_history_alcohol', 'social_history_sexual_activity', 'social_history_comment', 'medications','medications_main',
    ...
    public function get_studentnotificationpreview($id){

            //Student cannot preview deleted patients
            Auth::user()->notifications->where('id',$id)->markAsRead();
            $notificationpatient = Auth::user()->notifications->where('id',$id)->first();
            ...
            $id = $notificationpatient['data']['patientid'];
            $patient = patient::where('patient_id', $id)->first();
            if($patient->archived)
            {
                $error_message = "You cannot preview deleted patient.";
    ...
    public function get_preview($id){
            ...
            elseif (!$patient->completed_flag) {
                ...
                $feedback = users_patient::where('patient_id',$id)->
                where('user_id',$instructor_id)->pluck('feedback');
                ...
                return view('patient/preview', compact('student_details','instructor_Details', 'patient', 'navs', 'feedback',
                    'vital_signs_header', 'HPI', 'diagnosis_list_surgical_history', 'surgical_history_comment',
                    'diagnosis_list_personal_history', 'personal_history_comment', 'family_members_details',
                    'comment_family_history', 'social_history_smoke_tobacco', 'social_history_non_smoke_tobacco',
                    'social_history_alcohol', 'social_history_sexual_activity', 'social_history_comment', 'medications','medications_main',
                    ...
                    'cardiovascular_symptoms', 'cardiovascular_comment', 'respiratory_symptoms', 'respiratory_comment',
                    'eyes_symptoms', 'eyes_comment', 'hent_symptoms', 'hent_comment', 'constitutional_symptoms',
                    'constitutional_comment', 'comment_order', 'labs', 'procedures', 'images', 'results','mdm','diagnosis_list_disposition','disposition_value','disposition_comment','dosage','i','diagnosis_list_ddx','for_review_patients'));
            }

    }

    public function get_notification_preview($id){
            //Student cannot preview deleted patients
            Auth::user()->notifications->where('id',$id)->markAsRead();
            $notificationpatient = Auth::user()->notifications->where('id',$id)->first();
            Log::info("nenu");
            Log::info($notificationpatient);
            $readnotifications = Auth::user()->readNotifications;
            $i=0;
            foreach($readnotifications as $readnotification) {
                $i = $i + 1;
